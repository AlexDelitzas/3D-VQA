"""
Training Script to be called from cmd in project_root directory
"""
...
from typing import Optional, OrderedDict, Dict, Tuple, List, Union, Any
...
from models.model_utils import (_print_state_dict_shapes,
                                init_model_from_weights,
                                init_votenet_from_weights,
                                get_num_params)
...
def parse_option():
    ...
    parser.add_argument("--pretrain_src", type=str, help="Name of directory with pretrained ckpt", default="None")

    parser.add_argument("--debug", action="store_true", help="debugging mode")
    parser.add_argument("--tag", type=str, help="tag for the training, e.g. XYZ_COLOR", default="")
    parser.add_argument("--gpu", type=str, help="gpu", default="0")
    # Training
    parser.add_argument("--cur_criterion", type=str, default="answer_acc_at10", help="data augmentation type")
    ...
    parser.add_argument('--lr_decay_step', nargs='+', type=int, default=[15,23]) # 15
    ...
    # Data
    parser.add_argument("--num_points", type=int, default=30000, help="Point Number [default: 40000]")  # 40000 orig
    ...
    parser.add_argument("--seed_feat_dim", type=int, help="", default=512)  # 256 in the original
    parser.add_argument("--proposal_size", type=int, help="", default=128)  # 128 in the original
    parser.add_argument("--num_proposals", type=int, default=256, help="Proposal number [default: 256]")  # 256 orig
    ...
    ## MCAN
    parser.add_argument("--mcan_pdrop", type=float, help="", default=0.1)  # 0.1
    ...
    parser.add_argument("--mcan_num_layers", type=int, help="", default=2)  # 2, mcan: 6

    args = parser.parse_args()
    return args


def get_answer_cands(args: argparse,
                     scanqa
...
def get_dataloader(args: argparse,
                   ...
                   all_scene_list,
                   split: str,
                   config: ScannetQADatasetConfig,
                   augment
                   ) -> Tuple[ScannetQADataset, DataLoader]:
    ...
    elif 'clip' in args.tokenizer_name:
        tokenizer = CLIPTokenizer.from_pretrained("openai/clip-vit-base-patch32")
    else:
        tokenizer = None

    dataset = ScannetQADataset(scanqa=scanqa[split],
...
def get_model(args: argparse,
    ...
    if "bert-" in args.tokenizer_name:
        ...
        clip_model_name = args.tokenizer_name
        lang_emb_size = 512
    else:
        lang_emb_size = 300  # glove emb_size

    # initiate model
    input_channels = (int(args.use_multiview) * 128
                      + int(args.use_normal) * 3
                      + int(args.use_color) * 3
                      + int(not args.no_height))

    model = ScanQA(num_answers=config.num_answers,
                   # proposal
    ...
    # Load Pretrained Weights if specified in argparser
    if args.pretrain_src != "None" and args.pretrain_src is not None:
        pretrained_path = os.path.join('./outputs', args.pretrain_src + '/', 'checkpoint.tar')
        if not os.path.isfile(pretrained_path):
            pretrained_path = os.path.join('./outputs', args.pretrain_src + '/', 'model_last.pth')
            if not os.path.isfile(pretrained_path):
                raise ValueError("Checkpoint File doesn't exist")
        model = init_model_from_weights(model, torch.load(pretrained_path))
...
def get_scanqa(scanqa_train: List[Dict[str, Any]],
               scanqa_val: List[Dict[str, Any]],
               train_num_scenes: int,
               val_num_scenes: int):
...
def train(args: argparse):
    ...
    scanqa_train, scanqa_val, all_scene_list = get_scanqa(SCANQA_TRAIN,
                                                          SCANQA_VAL,
                                                          args.train_num_scenes,
                                                          args.val_num_scenes)
    scanqa = {"train": scanqa_train,
              "val": scanqa_val}
