...
from lib.random_cuboid import RandomCuboid
...
class ScannetQADataset(Dataset):
    def __init__(self, scanqa, scanqa_all_scene, 
        ...
        if split == 'train':
            self.use_random_cuboid = True
        else:
            self.use_random_cuboid = False
        self.random_cuboid_augmentor = RandomCuboid(min_points=30000)
        ...
        if tokenizer is None:
            ...
            self.use_clip_embeds = True
            for record in self.scanqa:
                record.update(token=tokenizer(record['question'],return_tensors='np'))
                token=tokenizer(record['question'],return_tensors='pt')
                #print(token)
    ...
    def __getitem__(self, idx):
        ...
        #
        if self.use_bert_embeds or self.use_clip_embeds:
        ...
        if self.split != 'test':
            ...
            for i, gt_id in enumerate(instance_bboxes[:num_bbox,-1]): 
                if gt_id == object_ids[0]:
                    ref_center_label = instance_bboxes[i, 0:3]
            ...
            if self.augment and self.use_random_cuboid:
                (
                    ...
                    point_cloud, instance_bboxes, ref_center_label,[instance_labels, semantic_labels]
                )
...
#       object_names = None if object_names is None else object_names
#         if (object_names):
#             mask = [self.raw2label[object_name] if object_name in self.raw2label else 17 for object_name in object_names]
#             object_cat = np.zeros(18)
#             object_cat[mask] = 1
        #print(object_cat)
    ...
    def _load_data(self):
        ...
        if self.use_bert_embeds or self.use_clip_embeds:
            ...
            raise NotImplementedError("Use either BERT or CLIP")
