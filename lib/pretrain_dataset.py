...
class PretrainDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.topdown_imgs = topdown_imgs
    ...
    def __getitem__(self, idx):
        ...
        scene_path = os.path.join(CONF.PATH.DATA,'scannet/scene_types_all.txt')
        if os.path.isfile(scene_path):
            scene_type_mapping = read_scene_types_mapping(scene_path, remove_spaces=True)
            type_name = get_field_from_info_file(os.path.join(CONF.PATH.DATA, 'scannet/scans/' + scene_id + '/' + scene_id +'.txt'), 'sceneType')
        ...
        data_dict["lang_tokens"] = lang_tokens #.astype(np.int64) # language tokens
        ...
        if os.path.isfile(scene_path):
            data_dict["scene_type"] = scene_type
        data_dict["scene_id"] =  int(scene_id.split("scene0")[1].split('_')[0])
    ...
    def _tranform_des(self):
        ...
        for data in self.scanrefer:
            ...
            tokens =  "Point cloud of a scene where "+ data["description"]
            
            # store
            lang[scene_id][object_id][ann_id] = tokens
