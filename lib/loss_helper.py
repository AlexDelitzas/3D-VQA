...
def compute_clip_losses(data_dict: Dict[str, Union[torch.Tensor, float]],
                        loss_weights: Dict[str, float]
                        ) -> torch.Tensor:
    assert loss_weights.get("text_loss") is not None and loss_weights.get("text_loss") != 0, \
           "Text Loss weight is not set"
    assert loss_weights.get("image_loss") is not None and loss_weights.get("image_loss") != 0, \
           "Image Loss weight is not set"

    # Compute normed embedding-vectors
    scene_features_norm = data_dict["scene_rep"] / data_dict["scene_rep"].norm(dim=-1, keepdim=True)
    text_features_norm = data_dict["text_rep"] / data_dict["text_rep"].norm(dim=-1, keepdim=True)
    image_features_norm = data_dict["image_rep"] / data_dict["image_rep"].norm(dim=-1, keepdim=True)

    # Compute Similarities and Losses and incorporate weights
    t_cos = cosine_sim(scene_features_norm, text_features_norm)
    t_cosine_loss = (1 - t_cos).mean() * loss_weights.get("text_loss")

    i_cos = cosine_sim(scene_features_norm, image_features_norm)
    i_cosine_loss = (1 - i_cos).mean() * loss_weights.get("image_loss")
    return t_cosine_loss + i_cosine_loss
...
def get_loss(data_dict: Dict[str, Union[torch.Tensor, float]],
             config,
             detection: bool = True,
             use_reference: bool = False,
             use_lang_classifier: bool = False,
    ...
    #data_dict["clip_loss"] = compute_clip_losses(data_dict)
...
def get_pretrain_loss(data_dict: Dict[str, Union[torch.Tensor, float]],
                     config,
                     detection: bool = True,
                     use_reference: bool = False,
                     use_lang_classifier: bool = False,
    ...
    data_dict["clip_loss"] = compute_clip_losses(data_dict, loss_weights)

    loss = (loss_weights.get("object_detection_loss", 1.) * (
                loss_weights.get('vote_loss', 1.) * data_dict['vote_loss']
                + loss_weights.get('objectness_loss', 1.) * data_dict['objectness_loss']
                + loss_weights.get('box_loss', 1.) * data_dict['box_loss']
                + loss_weights.get('sem_cls_loss', 1.) * data_dict['sem_cls_loss']
                + loss_weights.get('ref_loss', 1.) * data_dict["ref_loss"]
           ...
           + loss_weights.get('clip_loss', 1.) * data_dict["clip_loss"])
