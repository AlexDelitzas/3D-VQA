"""
-> Copied and modified from: https://github.com/ATR-DBI/ScanQA/blob/main/models/proposal_module.py
    -> Which was(Modified from: https://github.com/facebookresearch/votenet/blob/master/models/proposal_module.py)
"""
...
from typing import OrderedDict, Dict, Tuple, List, Union, Optional
...
class ProposalModule(nn.Module):
    ...
    def forward(self,
        ...
        Args:
            xyz: (B,K,3)
            features: (B,C,K)
            data_dict: Data Dictionary containing all the in- and outputs
        Returns:
            scores: (B,num_proposal,2+3+NH*2+NS*4) 
        """

        # Farthest point sampling (FPS) on votes
        # featurea: batch, 256, 1024
        xyz, features, fps_inds = self.vote_aggregation(xyz, features)  # batch, votenet_mlp_size (128), 256
        
        sample_inds = fps_inds
        data_dict['aggregated_vote_xyz'] = xyz  # (N, num_proposal, 3)
        data_dict['aggregated_vote_features'] = features.permute(0, 2, 1).contiguous()  # (N, num_proposal, 128)
        data_dict['aggregated_vote_inds'] = sample_inds  # (N, num_proposal,) # should be 0,1,2,...,num_proposal

        # --------- PROPOSAL GENERATION ---------
        net = F.relu(self.bn1(self.conv1(features))) 
        net = F.relu(self.bn2(self.conv2(net))) 
        net = self.conv3(net)  # (N, 2+3+num_heading_bin*2+num_size_cluster*4, num_proposal)
    ...
    def decode_pred_box(self, data_dict: Dict[str, torch.Tensor]):
        # predicted bbox
